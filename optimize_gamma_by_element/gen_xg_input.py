...
parser.add_argument('-b', '--basis', type=str, default='awcvtz')
parser.add_argument('-c', '--charge', type=int, default=0)

def arr_to_molpro_string(arr, width=4, prec=2):
    return "[" + ','.join(f'{num:{width}.{prec}}' for num in arr) + "]"
...
def main(args):
    ...
    if args.outfile is None:
        inp_params['outfile'] = "table_{atom}{charge}.csv".format(**inp_params)
    else:
